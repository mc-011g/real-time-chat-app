...
export default function Tooltip({ extraClasses, position, text, children, id }: { extraClasses?: string, position: string, text: string, children: React.ReactNode, id: string }) {
  ...
  return (
    <div className="flex justify-center">
      ...
      <div
        ...
        onMouseOver={() => isHovering = true} onMouseOut={() => isHovering = false}
      ...
      {show &&
        <div className={`${extraClasses} transistion absolute w-fit h-fit z-100 bg-gray-900 rounded-lg px-2 py-1 flex
         ${position === "bottom" ? "top-[1.5lh]" : ""}
          ${position === "top" ? "bottom-[1.5lh]" : ""}
            ${position === "left" ? "right-[1.5lh]" : ""}
           `}>
          <div className="relative flex justify-center">
            <div className="text-gray-50" role="tooltip" id={`tooltipContent${id}`}>{text}</div>
